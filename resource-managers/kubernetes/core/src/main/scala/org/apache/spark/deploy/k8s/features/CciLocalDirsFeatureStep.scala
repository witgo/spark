...
private[spark] class CciLocalDirsFeatureStep(
  conf: KubernetesConf,
  ...
  // property - we want to instead default to mounting an localdir volume that doesn't already
  // exist in the image.
    ...
    val localDirVolumes = resolvedLocalDirs
      ...
      .map { case (localDir, index) =>
        new VolumeBuilder()
          ...
          .withNewLocalDir()
          .withSizeLimit(new Quantity(quota))
          .endLocalDir()
          .build()
      }
