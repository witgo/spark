...
private[spark] class EvsDirsFeatureStep(
  ...
  def resourcesNamePrefix(index: Int): String = {
    conf match {
      case _: KubernetesDriverConf =>
        s"${conf.resourceNamePrefix}-driver-spark-local-dir-${index + 1}"
      case executorConf: KubernetesExecutorConf =>
        val executorId = executorConf.executorId
        s"${conf.resourceNamePrefix}-executor-$executorId-spark-local-dir-${index + 1}"
    }
  }
