...
class FFNClassifierModel private[mllib](val annModel: FeedForwardModel,
                                        val labelToIndex: Map[Double, Int])
  extends ClassificationModel with FFNClassifierHelper with Serializable {
  /**
...
class FFNClassifier private(val labelToIndex: Map[Double, Int],
                            private val conf: FeedForwardTopology,
                            private val initialWeights: Vector,
                            private val maxIterations: Int,
                            private val stepSize: Double,
                            private val convergeTol: Double)
  extends FFNClassifierHelper with Serializable {

  def run(data: RDD[LabeledPoint], batchSize: Int = 1): FFNClassifierModel = {
    val annData = data.map(lp => labeledPointToVectorPair(lp))
    /* train the model */
    val model = FeedForwardNetwork.train(annData, batchSize, maxIterations, conf, initialWeights)
    new FFNClassifierModel(model, labelToIndex)
  }
}
...
object FFNClassifier {
  ...
  def train(data: RDD[LabeledPoint],
            ...
            maxIterations: Int,
            convergenceTol: Double): FFNClassifierModel = {
  ...
  def train(data: RDD[LabeledPoint],
            batchSize: Int,
            model: FFNClassifierModel,
            maxIterations: Int,
            convergenceTol: Double): FFNClassifierModel = {
    val initialWeights = FeedForwardModel.rollWeights(model.annModel.weights, model.annModel.bias)
    new FFNClassifier(model.labelToIndex, model.annModel.config, initialWeights, maxIterations,
      defaultStepSize, convergenceTol).run(data, batchSize)
  }
  ...
  def train(data: RDD[LabeledPoint],
    ...
    new FFNClassifier(labelToIndex,
      FeedForwardTopology.multiLayerPerceptron(numIn +: hiddenLayersTopology :+ numOut),
  ...
  def train(data: RDD[LabeledPoint],
            ...
            stepSize: Double,
            convergenceTol: Double): FFNClassifierModel = {
    val initialWeights = FeedForwardModel.rollWeights(model.annModel.weights,
      model.annModel.bias)
    new FFNClassifier(model.labelToIndex, model.annModel.config,
   ...
   * @return ANN classifier model.
   */
  def train(data: RDD[LabeledPoint]): FFNClassifierModel = {
  ...
  def randomWeights(data: RDD[LabeledPoint],
    ...
    /* TODO: remove duplication - the same analysis will be done in FFNClassifier.run() */
