...
import org.apache.spark.network.buffer.{ChunkedByteBuffer, InputStreamManagedBuffer, ManagedBuffer}
...
private[spark] class NettyBlockTransferService(
  ...
  override def uploadBlock(
    ...
    val uploadBlock = new UploadBlock(appId, execId, blockId.toString, metadata.toArray, blockData)
    val encodedLength = uploadBlock.encodedLength() + 1
    val isBodyInFrame = encodedLength < 48 * 1024 * 1024
    val message = new InputStreamManagedBuffer(uploadBlock.toInputStream, encodedLength, true)
    client.sendRpc(message, isBodyInFrame, new RpcResponseCallback {
